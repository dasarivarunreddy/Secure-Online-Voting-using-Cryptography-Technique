...
$sq = mysqli_query($con, 'SELECT username FROM loginusers WHERE username="'.sha1($_POST['username']).'"');
...
$sql = mysqli_query($con, 'INSERT INTO voters(firstname,lastname,username)
         VALUES("'.sha1($_POST['firstname']).'","'.sha1($_POST['lastname']).'","'.sha1($_POST['username']).'")');
		 if (!$sql) { 
		 die (mysqli_error($con));
		 }
$sql2 = mysqli_query($con, 'INSERT INTO loginusers(username,password)
         VALUES("'.sha1($_POST['username']).'","'.md5($_POST['password']).'")'); 
if (!$sql2) { 
		 die (mysqli_error($con));
		 }
else {
